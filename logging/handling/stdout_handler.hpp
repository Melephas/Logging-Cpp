...
namespace logging::handling {
    ...
    public:
        stdout_handler();
        stdout_handler(logging::level level, std::vector<std::unique_ptr<filter::filter>> filters, std::unique_ptr<format::formatter> formatter);
        ~stdout_handler() override = default;

        // Outputs the formatted record to stdout if it passes all the filters
        void dispatch_record(record record) const noexcept override;
    };
}
